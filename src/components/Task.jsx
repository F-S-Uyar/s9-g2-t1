...
const Task = ({ taskObj, onComplete }) => {
  const deadline = newDate(taskObj.deadline);
  const terminText = formatDistanceToNow(deadline, {
    locale: tr,
    addSuffix: true,
  }) 

  const terminClass = differenceInDays(deadline, newDate()) < 3 ? 'bg-urgent' : 'bg-normal';
  ...
  return (
    <div className="p-6 bg-white rounded-md leading-normal mt-4 shadow-[0_4px_5px_0_rgb(0,0,0,0.1)]">
      ...
      <div className="text-xs pt-1">
        son teslim: <span className={`${terminClass} py-1 px-2 rounded-sm inline-block`}>{taskObj.deadline}</span>
      </div>
